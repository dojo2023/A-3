package model;

import java.io.Serializable;

public class Registers implements Serializable {
	private String id;
	private int outer;
	private int tops;
	private int bottoms;
	private int dress;
	private int socks;
	private int shose;
	private int acce;
	private int spring;
	private int summer;
	private int autum;
	private int winter;
	private int cute;
	private int casual;
	private int simple;
	private int stripe;
	private int check;
	private int dot;
	private int beauty;
	private int mode;
	private int natural;
	private int conserva;
	private int cool;
	private int lowheight;
	private int mens;
	private int lowprice;
	private int monotone;
	private int skewave;
	private int skestraight;
	private int skenatural;
	private int repeat;
	private String img;


	public Registers(String id, int outer, int tops, int bottoms, int dress, int socks, int shose, int acce, int spring,
			int summer, int autum, int winter, int cute, int casual, int simple, int stripe, int check, int dot,
			int beauty, int mode, int natural, int conserva, int cool, int lowheight, int mens, int lowprice,
			int monotone, int skewave, int skestraight, int skenatural, int repeat, String img) {
		super();
		this.id = id;
		this.outer = outer;
		this.tops = tops;
		this.bottoms = bottoms;
		this.dress = dress;
		this.socks = socks;
		this.shose = shose;
		this.acce = acce;
		this.spring = spring;
		this.summer = summer;
		this.autum = autum;
		this.winter = winter;
		this.cute = cute;
		this.casual = casual;
		this.simple = simple;
		this.stripe = stripe;
		this.check = check;
		this.dot = dot;
		this.beauty = beauty;
		this.mode = mode;
		this.natural = natural;
		this.conserva = conserva;
		this.cool = cool;
		this.lowheight = lowheight;
		this.mens = mens;
		this.lowprice = lowprice;
		this.monotone = monotone;
		this.skewave = skewave;
		this.skestraight = skestraight;
		this.skenatural = skenatural;
		this.repeat = repeat;
		this.img = img;
	}


	public Registers() {
		super();
		this.id = "";
		this.outer = 0;
		this.tops = 0;
		this.bottoms = 0;
		this.dress = 0;
		this.socks = 0;
		this.shose = 0;
		this.acce = 0;
		this.spring = 0;
		this.summer = 0;
		this.autum = 0;
		this.winter = 0;
		this.cute = 0;
		this.casual = 0;
		this.simple = 0;
		this.stripe = 0;
		this.check = 0;
		this.dot = 0;
		this.beauty = 0;
		this.mode = 0;
		this.natural = 0;
		this.conserva = 0;
		this.cool = 0;
		this.lowheight = 0;
		this.mens = 0;
		this.lowprice = 0;
		this.monotone = 0;
		this.skewave = 0;
		this.skestraight = 0;
		this.skenatural = 0;
		this.repeat = 0;
		this.img = "";
	}


	public String getId() {
		return id;
	}


	public void setId(String id) {
		this.id = id;
	}


	public int getOuter() {
		return outer;
	}


	public void setOuter(int outer) {
		this.outer = outer;
	}


	public int getTops() {
		return tops;
	}


	public void setTops(int tops) {
		this.tops = tops;
	}


	public int getBottoms() {
		return bottoms;
	}


	public void setBottoms(int bottoms) {
		this.bottoms = bottoms;
	}


	public int getDress() {
		return dress;
	}


	public void setDress(int dress) {
		this.dress = dress;
	}


	public int getSocks() {
		return socks;
	}


	public void setSocks(int socks) {
		this.socks = socks;
	}


	public int getShose() {
		return shose;
	}


	public void setShose(int shose) {
		this.shose = shose;
	}


	public int getAcce() {
		return acce;
	}


	public void setAcce(int acce) {
		this.acce = acce;
	}


	public int getSpring() {
		return spring;
	}


	public void setSpring(int spring) {
		this.spring = spring;
	}


	public int getSummer() {
		return summer;
	}


	public void setSummer(int summer) {
		this.summer = summer;
	}


	public int getAutum() {
		return autum;
	}


	public void setAutum(int autum) {
		this.autum = autum;
	}


	public int getWinter() {
		return winter;
	}


	public void setWinter(int winter) {
		this.winter = winter;
	}


	public int getCute() {
		return cute;
	}


	public void setCute(int cute) {
		this.cute = cute;
	}


	public int getCasual() {
		return casual;
	}


	public void setCasual(int casual) {
		this.casual = casual;
	}


	public int getSimple() {
		return simple;
	}


	public void setSimple(int simple) {
		this.simple = simple;
	}


	public int getStripe() {
		return stripe;
	}


	public void setStripe(int stripe) {
		this.stripe = stripe;
	}


	public int getCheck() {
		return check;
	}


	public void setCheck(int check) {
		this.check = check;
	}


	public int getDot() {
		return dot;
	}


	public void setDot(int dot) {
		this.dot = dot;
	}


	public int getBeauty() {
		return beauty;
	}


	public void setBeauty(int beauty) {
		this.beauty = beauty;
	}


	public int getMode() {
		return mode;
	}


	public void setMode(int mode) {
		this.mode = mode;
	}


	public int getNatural() {
		return natural;
	}


	public void setNatural(int natural) {
		this.natural = natural;
	}


	public int getConserva() {
		return conserva;
	}


	public void setConserva(int conserva) {
		this.conserva = conserva;
	}


	public int getCool() {
		return cool;
	}


	public void setCool(int cool) {
		this.cool = cool;
	}


	public int getLowheight() {
		return lowheight;
	}


	public void setLowheight(int lowheight) {
		this.lowheight = lowheight;
	}


	public int getMens() {
		return mens;
	}


	public void setMens(int mens) {
		this.mens = mens;
	}


	public int getLowprice() {
		return lowprice;
	}


	public void setLowprice(int lowprice) {
		this.lowprice = lowprice;
	}


	public int getMonotone() {
		return monotone;
	}


	public void setMonotone(int monotone) {
		this.monotone = monotone;
	}


	public int getSkewave() {
		return skewave;
	}


	public void setSkewave(int skewave) {
		this.skewave = skewave;
	}


	public int getSkestraight() {
		return skestraight;
	}


	public void setSkestraight(int skestraight) {
		this.skestraight = skestraight;
	}


	public int getSkenatural() {
		return skenatural;
	}


	public void setSkenatural(int skenatural) {
		this.skenatural = skenatural;
	}


	public int getRepeat() {
		return repeat;
	}


	public void setRepeat(int repeat) {
		this.repeat = repeat;
	}


	public String getImg() {
		return img;
	}


	public void setImg(String img) {
		this.img = img;
	}




}